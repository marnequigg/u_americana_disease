...
*We decided to remove this confidence scheme by adding the phylogeny analysis
...
As a way to verify the pathogen detection, I created a phylogeny of the pathogens from each disease. I subsetted the reads from the alignment to the elm genome and the three diseases of interest into files for each disease. The scripts included here are for mal secco. The first script is 06.phylo_gene01.sh. This extracts the reads from the gene of interest, calls varients, creates a consensus sequence, and creates the tree. If you want to do multiple genes, you run this script for each gene of interest then combine them with the 07.fasta_merge.py. This script is from Hybpiper and takes the two fasta files with the same individuals (headers must be identical) and combines them. Then you can run IQtree again on that combined fasta file with the same line of code that is in 06.phylo_gene01.sh. For visualization, all the cladograms were made in RStudio and included in the script 08.phylo_visualization.Rmd. There is code to make various types of cladograms, including ones where I add tips based of latitude, longitude, and year.
